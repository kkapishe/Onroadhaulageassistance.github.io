...
$tdate=date('Y-m-d');
$query=mysqli_query($con,"select sum(AssistanceCost)  as amount from tbl_invoice where (date_invoiced)='$tdate' && (UserId='$userid');");
...
$query1=mysqli_query($con,"select sum(AssistanceCost)  as yesterdayassistance from tbl_invoice where (AssistanceDate)='$ydate' && (UserId='$userid');");
...
$crrntdte=date("Y-m-d");
$query2=mysqli_query($con,"select sum(AssistanceCost)  as weeklyassistances from tbl_invoice where ((AssistanceDate) between '$pastdate' and '$crrntdte') && (UserId='$userid');");
...
$crrntdte=date("Y-m-d");
$query3=mysqli_query($con,"select sum(AssistanceCost)  as monthlyassistnce from tbl_invoice where ((AssistanceDate) between '$monthdate' and '$crrntdte') && (UserId='$userid');");
...
$userid=$_SESSION['detsuid'];
 $cyear= date("Y");
$query4=mysqli_query($con,"select sum(ExpenseCost)  as sum_yearly_assistance from tbl_invoice
...
//Yearly Expense
$userid=$_SESSION['detsuid'];
$query5=mysqli_query($con,"select sum(AssistanceCost)  as sum_total_assistance from tbl_invoice where UserId='$userid';");
